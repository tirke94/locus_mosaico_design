...
const Nav = () => {
    ...
    // const [lengthCartItems, setLengthCartItems] = useState(0)

    const numberOfCartProduct = () => {
        let tmp = []
        let num
        if (JSON.parse(localStorage.getItem('cart')) != null) {
            tmp = [...JSON.parse(localStorage.getItem('cart'))]
            num = tmp.length
            // setLengthCartItems(+num)
            setNumCartItems(+num)
    ...
    return (
        <div className='nav'>
            ...
            <NavLink to='/cart' id='cart-small-device'>
                ...
                <div className='cart_div_items'>
                    <span className='cart_num_items'>{numCartItems}</span>
                </div>
            </NavLink>
